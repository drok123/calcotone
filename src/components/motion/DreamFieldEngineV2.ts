...
export class DreamFieldEngine {
  ...
  render(ctx: CanvasRenderingContext2D, frame: DreamFrame) {
    ...
    if (this.raster) {
      ...
      ctx.drawImage(this.raster, 0, 0, RASTER_W, RASTER_H, 0, 0, width, height);
      ctx.restore();
    }
  ...
  private renderRaster(
    ...
    for (let py = 0; py < RASTER_H; py += 1) {
      for (let px = 0; px < RASTER_W; px += 1) {
        ...
        const warp = fbm(nx * 1.25 + t * 0.35, ny * 1.25 - t * 0.22) - 0.5;
        const warp2 = fbm(nx * 2.4 - t * 0.18, ny * 2.1 + t * 0.28) - 0.5;
        ...
        const organicTexture = fbm(x * 3.1 + t * 0.4, y * 3.1 - t * 0.3);
        const dreamFog = smoothstep(0.32, 0.86, organicTexture) * (0.12 + atmos * 0.20 + halo * 0.10);
        intensity = clamp01(intensity * 1.24 + dreamFog + patchEnergy * 0.035);

        const radial = Math.hypot(nx / Math.max(0.01, aspect), ny);
        const vignette = 1 - smoothstep(0.72, 1.32, radial);
        intensity *= 0.55 + vignette * 0.72;
        ...
        const totalTint = Math.max(1, 1 + copperBias + seaBias + duskBias);

        let r = (PALETTE.bone[0] + PALETTE.copper[0] * copperBias + PALETTE.sea[0] * seaBias + PALETTE.dusk[0] * duskBias) / totalTint;
        let g = (PALETTE.bone[1] + PALETTE.copper[1] * copperBias + PALETTE.sea[1] * seaBias + PALETTE.dusk[1] * duskBias) / totalTint;
        let b = (PALETTE.bone[2] + PALETTE.copper[2] * copperBias + PALETTE.sea[2] * seaBias + PALETTE.dusk[2] * duskBias) / totalTint;

        const sparkle = galaxy > 0.55 && hash(px * 0.73 + Math.floor(time * 2), py * 1.13) > 0.982 ? 0.8 : 0;
        const hot = clamp01(intensity + sparkle);
        const base = 8 + intensity * 28;
        r = base + r * hot * 0.82;
        g = base + g * hot * 0.82;
        b = base + b * hot * 0.82;
  ...
  private eyeField(x: number, y: number, t: number, ember: number, halo: number) {
    ...
    const spokes = Math.pow(Math.max(0, Math.sin(angle * 24 + r * 19 - t * 2.2)), 8) * (1 - smoothstep(0.20, 0.48, r));
    return clamp01(lidLine * 0.55 + irisRing * 0.88 + pupil * (0.45 + ember * 0.35) + spokes * 0.25);
  }
  ...
  private treeField(x: number, y: number, t: number, atmos: number) {
    ...
    for (let level = 0; level < 4; level += 1) {
      const yy = -0.05 - level * 0.18;
      const spread = 0.20 + level * 0.13 + atmos * 0.08;
      const local = Math.abs(y - yy);
      const branchMask = 1 - smoothstep(0.015, 0.055, local);
      const left = 1 - smoothstep(0.025, 0.075, Math.abs(x + spread * (0.45 + local * 2.2) + Math.sin(t + level) * 0.025));
      const right = 1 - smoothstep(0.025, 0.075, Math.abs(x - spread * (0.45 + local * 2.2) - Math.cos(t * 0.8 + level) * 0.025));
      branches = Math.max(branches, branchMask * Math.max(left, right));
    }

    const canopyNoise = fbm(x * 5 + t * 0.2, y * 5 - t * 0.14);
    const canopyShape = 1 - smoothstep(0.48, 0.82, Math.hypot(x * 0.82, (y + 0.33) * 1.3));
    const canopy = canopyShape * smoothstep(0.42, 0.69, canopyNoise) * (0.35 + atmos * 0.42);
    return clamp01((trunk * heightMask + branches * 0.82 + canopy) * (ground ? 0.88 : 1));
  }
  ...
  private oceanField(x: number, y: number, t: number, drift: number) {
    ...
    return clamp01(horizon * 0.72 + foam + smoothstep(0.56, 0.84, fbm(x * 4, y * 4 + t)) * waterMask * 0.18);
  }
  ...
  private galaxyField(x: number, y: number, t: number, halo: number, atmos: number) {
    ...
    const dust = smoothstep(0.50, 0.78, fbm(x * 6 + t * 0.2, y * 6 - t * 0.17)) * (1 - smoothstep(0.2, 1.1, r));
    return clamp01(core * 0.72 + arm * 0.82 + dust * 0.24);
  }
  ...
  private crystalField(x: number, y: number, t: number, grain: number) {
    ...
    const shardNoise = smoothstep(0.60, 0.84, fbm(x * 8 - t * 0.3, y * 8 + t * 0.25));
    return clamp01(edges * (0.40 + grain * 0.55) + shardNoise * grain * 0.38);
  }

  private landscapeField(x: number, y: number, t: number, atmos: number, drift: number) {
    const ridgeNoise = fbm(x * 1.55 + t * 0.12, 1.7 + t * 0.04);
    const detail = fbm(x * 4.8 - t * 0.05, 4.1);
    const ridgeY = -0.10 + (ridgeNoise - 0.5) * (0.58 + atmos * 0.22) + (detail - 0.5) * 0.16;
    const ridge = 1 - smoothstep(0.015, 0.075, Math.abs(y - ridgeY));

    const farNoise = fbm(x * 1.1 - t * 0.08, 7.2);
  ...
  private drawRecognitionAccents(ctx: CanvasRenderingContext2D, time: number, width: number, height: number, ember: number, halo: number, atmos: number) {
    ...
    if (this.weights.landscape > 0.46) {
      ctx.strokeStyle = `rgba(${PALETTE.ash.join(',')},0.11)`;
      ctx.beginPath();
      for (let i = 0; i <= 60; i += 1) {
        const x = i / 60 * width;
        const nx = i / 60 * 2 - 1;
        const y = height * (0.43 + (fbm(nx * 1.5 + time * 0.02, 3.2) - 0.5) * (0.18 + atmos * 0.05));
  ...
  private drawMemory(ctx: CanvasRenderingContext2D, time: number, cx: number, cy: number, width: number, height: number, halo: number, artifact: number) {
    ...
    ctx.save();
    ctx.globalCompositeOperation = 'screen';
    ctx.globalAlpha = 0.10 + halo * 0.09 + artifact * 0.04;
    const breathe = 1.004 + Math.sin(time * 0.08) * 0.004 + halo * 0.005;
    ctx.translate(cx, cy);
    ctx.rotate(Math.sin(time * 0.027) * 0.004 + artifact * Math.sin(time * 0.5) * 0.002);
    ctx.scale(breathe, breathe);
    ctx.translate(-cx + Math.sin(time * 0.11) * artifact * 3, -cy);
