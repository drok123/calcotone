...
const MAX_CHARACTER_CURVE_CACHE = 192;
function getCharacterCurve(character: number, config: DelayAlgorithmConfig): Float32Array<ArrayBuffer> { const quantized = Math.round(character * 96) / 96; const key = `${config.id}:${quantized}`; const cached = CHARACTER_CURVE_CACHE.get(key); if (cached) return cached; const curve = createCharacterCurve(quantized, config); if (CHARACTER_CURVE_CACHE.size >= MAX_CHARACTER_CURVE_CACHE) { const oldest = CHARACTER_CURVE_CACHE.keys().next().value; if (oldest !== undefined) CHARACTER_CURVE_CACHE.delete(oldest); } CHARACTER_CURVE_CACHE.set(key, curve); return curve; }
