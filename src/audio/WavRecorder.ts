...
export class WavRecorder {
  ...
  private masterMode: RecorderMasterMode = 'clean';
  ...
  public setMasterMode(mode: RecorderMasterMode): void { this.masterMode = mode; }
  public getMasterMode(): RecorderMasterMode { return this.masterMode; }
  ...
  public start(): void {
    ...
    processor.port.onmessage = (event: MessageEvent<{ type?: string; left?: Float32Array; right?: Float32Array; peak?: number }>) => {
      ...
      if (data?.type === 'chunk' && data.left && data.right) {
        this.leftChunks.push(data.left); this.rightChunks.push(data.right);
      ...
      if (data?.type === 'stopped') { this.stopResolver?.(); this.stopResolver = null; }
    };
    processor.onprocessorerror = () => {
      this.recording = false; this.stopResolver?.(); this.stopResolver = null;
      this.disconnectNodes(); this.resetBuffers();
      console.error('CALCOTONE recorder AudioWorklet stopped unexpectedly.');
    };
    this.source.connect(processor); processor.connect(silentOutput); silentOutput.connect(this.context.destination);
    this.processor = processor; this.silentOutput = silentOutput; this.recording = true;
    processor.port.postMessage({ type: 'start', maxFrames: Math.floor(MAX_RECORDING_SECONDS * this.context.sampleRate) });
  }
  ...
  public async stop(): Promise<RecordedWav> {
    ...
    await new Promise<void>((resolve) => {
      ...
      const finish = () => {
        ...
        if (timeout !== null) clearTimeout(timeout); timeout = null; resolve();
      };
      this.stopResolver = finish; processor.port.postMessage({ type: 'stop' });
      timeout = globalThis.setTimeout(finish, 250);
    });
    this.stopResolver = null; this.disconnectNodes();
    if (this.frameCount === 0) { this.resetBuffers(); throw new Error('The recording did not contain any audio frames.'); }
    ...
    const mastered = masterStereo(left, right, this.context.sampleRate, this.masterMode);
    const blob = this.masterMode === 'raw' ? rawBlob : encodePcm24Wave(mastered.left, mastered.right, this.context.sampleRate);
    const result: RecordedWav = {
      blob, rawBlob, durationSeconds: this.frameCount / this.context.sampleRate,
      sampleRate: this.context.sampleRate, channels: 2, bitDepth: 24,
      peak: mastered.peak, rawPeak, masterMode: this.masterMode, gainAppliedDb: mastered.gainAppliedDb,
    };
    this.resetBuffers();
    return result;
  }
  ...
  public cancel(): void {
    this.recording = false; this.stopResolver?.(); this.stopResolver = null;
    this.processor?.port.postMessage({ type: 'cancel' }); this.disconnectNodes(); this.resetBuffers();
  }
  public dispose(): void { this.cancel(); }

  private disconnectNodes(): void {
    if (this.processor) {
      this.processor.onprocessorerror = null; this.processor.port.onmessage = null;
      try { this.source.disconnect(this.processor); } catch { /* engine shutdown */ }
      this.processor.disconnect(); this.processor.port.close();
    }
    this.silentOutput?.disconnect(); this.processor = null; this.silentOutput = null;
  }
  private resetBuffers(): void { this.leftChunks = []; this.rightChunks = []; this.frameCount = 0; this.peak = 0; }
}

function masterStereo(left: Float32Array, right: Float32Array, sampleRate: number, mode: RecorderMasterMode) {
  if (mode === 'raw') return { left, right, peak: measurePeak(left, right), gainAppliedDb: 0 };
  ...
  const makeup = Math.min(maxMakeup, Math.max(0.25, rmsGain), peakGain * (mode === 'loud' ? 1.7 : 1.25));
  const drive = mode === 'loud' ? 1.55 : 1.18;
  const ceiling = targetPeak;
  let finalPeak = 0;
  for (let i = 0; i < outL.length; i += 1) {
    outL[i] = softLimit(outL[i] * makeup, drive, ceiling);
    outR[i] = softLimit(outR[i] * makeup, drive, ceiling);
    finalPeak = Math.max(finalPeak, Math.abs(outL[i]), Math.abs(outR[i]));
  }
  return { left: outL, right: outR, peak: finalPeak, gainAppliedDb: 20 * Math.log10(Math.max(1e-8, makeup)) };
}

function makeHighPassState(sampleRate: number, frequency: number) {
  const rc = 1 / (2 * Math.PI * frequency);
  const dt = 1 / sampleRate;
  return { alpha: rc / (rc + dt), left: { x: 0, y: 0 }, right: { x: 0, y: 0 } };
}
function highPass(input: number, state: { x: number; y: number }, alpha: number): number {
  const output = alpha * (state.y + input - state.x); state.x = input; state.y = output; return output;
}
function softLimit(sample: number, drive: number, ceiling: number): number {
  const normalized = Math.tanh(sample * drive) / Math.tanh(drive);
  return Math.max(-ceiling, Math.min(ceiling, normalized * ceiling));
}
...
function flattenChunks(chunks: Float32Array[], frameCount: number): Float32Array {
  ...
  for (const chunk of chunks) {
    const remaining = frameCount - offset; if (remaining <= 0) break;
    output.set(chunk.subarray(0, remaining), offset); offset += Math.min(chunk.length, remaining);
  }
  return output;
}
function encodePcm24Wave(left: Float32Array, right: Float32Array, sampleRate: number): Blob {
  const channelCount = 2, bytesPerSample = 3, blockAlign = channelCount * bytesPerSample;
  const dataBytes = left.length * blockAlign; const buffer = new ArrayBuffer(44 + dataBytes); const view = new DataView(buffer);
  ...
  view.setUint32(28, sampleRate * blockAlign, true); view.setUint16(32, blockAlign, true); view.setUint16(34, 24, true);
  writeAscii(view, 36, 'data'); view.setUint32(40, dataBytes, true); let offset = 44;
  for (let index = 0; index < left.length; index += 1) { offset = writePcm24(view, offset, left[index]); offset = writePcm24(view, offset, right[index]); }
  return new Blob([buffer], { type: 'audio/wav' });
}
function writePcm24(view: DataView, offset: number, sample: number): number {
  const dither = (Math.random() - Math.random()) / 8_388_608; const clamped = Math.max(-1, Math.min(1, sample + dither));
  const integer = clamped < 0 ? Math.round(clamped * 8_388_608) : Math.round(clamped * 8_388_607);
  view.setUint8(offset, integer & 0xff); view.setUint8(offset + 1, (integer >> 8) & 0xff); view.setUint8(offset + 2, (integer >> 16) & 0xff); return offset + 3;
}
