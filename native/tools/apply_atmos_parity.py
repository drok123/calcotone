#!/usr/bin/env python3
"""Generate native_rack.cpp with canonical Ember and Atmos processors wired live.

The legacy rack remains intentionally monolithic while modules are migrated. This
build-time transform replaces only completed parity modules and leaves every other
...
def main() -> int:
  ...
  void process(float* data, std::size_t frames, float rate) noexcept {
    ...
    processor.set_parameter("drive", p.value[1]);
    processor.set_parameter("tone", p.value[2]);
    processor.set_parameter("heat", p.value[3]);
    processor.set_parameter("character", p.value[4]);
    processor.set_parameter("dynamics", p.value[5]);
...
struct Grain {'''
    ...
    # Ember now requires the sample rate just like the other stateful processors.
    ...
    print(f"generated {output_path} with live Ember and Atmos parity processing")
    return 0
