#!/usr/bin/env python3
"""Route Ember mode 3 through the dedicated magnetic-core processor."""
...
def main() -> int:
    ...
    magnetic_include = '#include "calcotone/ember_magnetic_core_processor.hpp"\n'
    if magnetic_include not in source:
        if include_anchor not in source:
            raise RuntimeError("Ember parity include anchor was not found")
        source = source.replace(include_anchor, include_anchor + magnetic_include, 1)
  ...
  int active_mode{-1};
  explicit Ember(float rate) : processor(rate), magnetic(rate) {}
  void process(float* data, std::size_t frames, float) noexcept {
    ...
    if (mode != active_mode) {
      ...
      else if (active_mode == 3) processor.reset();
      active_mode = mode;
    }
...
float read_delay'''
    ...
    print(f"generated {output_path} with dedicated Ember magnetic-core routing")
    return 0
