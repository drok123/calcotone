...
for (const file of ['grain-processor.js', 'dream-buffer-processor.js', 'recorder-processor.js']) {
  ...
  if (!existsSync(path)) {
    problems.push(`missing worklet: public/${file}`);
    continue;
  }
  try {
    execFileSync(process.execPath, ['--check', path], { stdio: 'pipe' });
  } catch {
    problems.push(`invalid worklet syntax: public/${file}`);
  }
  const source = readFileSync(path, 'utf8');
  if (source.includes('performance.now()')) {
    problems.push(`AudioWorklet uses unsupported performance.now(): public/${file}`);
  }
}
...
if (!/<title>CALCOTONE<\/title>/.test(html)) problems.push('browser title is not CALCOTONE');
if (!readme.startsWith('# CALCOTONE')) problems.push('README is still template content');
