...
pattern = re.compile(
    r"struct NativePressure::Impl \{[\s\S]*?void NativePressure::process\(float\* data, std::size_t frames\) noexcept \{[\s\S]*?\n\}\n\nstruct NativeDreamBuffer::Impl"
)
...
struct NativeDreamBuffer::Impl'''
next_rack, count = pattern.subn(replacement, rack, count=1)
if count != 1:
    raise RuntimeError("missing embedded NativePressure block")
rack_path.write_text(next_rack, encoding="utf-8")
...
check(pressureWeb.includes("export const SIGNAL_LAB_MODES: SignalLabMode[] = ['fet', 'opto', 'varimu', 'vca'];"), 'pressure', 'four stable Pressure mode indices');
check(railC.includes("const PRESSURE_STYLE_LABELS = ['Punch', 'Level', 'Glue', 'Crush'] as const;"), 'pressure', 'four stable Pressure style indices');
for (const needle of [
