...
path = Path('src/ui/faceplateLayout.ts')
text = path.read_text()
start = text.find('function loadSavedLayout(): FaceplateLayout {')
end = text.find('\nfunction nearestWithin(', start)
if start < 0 or end < 0:
...
'''
path.write_text(text[:start] + loader + text[end:])
